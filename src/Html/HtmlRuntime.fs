...
module HtmlRuntime =
    ...
    let normalizeWs (str:String) =
        HtmlParser.wsRegex.Value.Replace(str.Replace('–', '-'), " ").Replace("[edit]", null).Trim()

    let getName defaultName (element:HtmlNode) (parents:HtmlNode list) = 
    ...
    let parseTable inferenceParameters includeLayoutTables makeUnique index (table:HtmlNode, parents:HtmlNode list) = 
        let rowSpan cell = 
    ...
    let parseList makeUnique index (list:HtmlNode, parents:HtmlNode list) =
    ...
    let parseDefinitionList makeUnique index (definitionList:HtmlNode, parents:HtmlNode list) =
...
type HtmlDocument (doc, tables, lists, definitionLists) =
...
type HtmlTable<'RowType> (name:string, headers:string[] option, values:'RowType[], html:HtmlNode) =
...
type HtmlList<'ItemType> (name:string, values:'ItemType[], html) = 
